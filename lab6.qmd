...
The DFBetas measures the difference between an estimated regression coefficient obtained from full data and the regression coefficient obtained when a particular observation is deleted. As a rule of thumb, an observation is considered influential if the absolute value of DFBETAS exceeds 1 for small to medium data and $\frac{2}{\sqrt(n)}$ for large data, where $n$ is a sample size (Kutner, 2005).
