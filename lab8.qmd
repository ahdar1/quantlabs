...
For example, The model correctly identifies:
...
As with multiple regression, we first inspect the p-values (or Sig.value).

Variables are significant when: $p < .05$. As you can see from the table above, all variables are significant except Gender. 

Therefore:

- Higher `Guilt` scores significantly increase the likelihood of joining the green loyalty programme (B = .541, Exp(B) = 1.718, p < .001).
- Higher `Attract` scores significantly increase the likelihood of joining the green loyalty programme (B = .872, Exp(B) = 2.392, p < .001).
- Older `Age` significantly decreases the likelihood of joining the green loyalty programme (B = -.034, Exp(B) = .967, p < .001).
- `Gender` does not significantly influence joining behaviour (B = .135, p = .207).

Thus, anticipated guilt, perceived attractiveness, and age are significant predictors of joining behaviour, whereas gender is not.

### Step 2: Interpret the Odds Ratios

The **Exp(B)** column reports the **odds ratios** associated with each predictor.

An odds ratio tells us how the odds of joining the green loyalty programme change when a predictor increases by one unit, while holding all other variables constant.

#### The Basic Rule:

| Exp(B) | Meaning |
|--------|---------|
| $> 1$ | Odds **increase** (positive relationship) |
| $= 1$ | No effect |
| $< 1$ | Odds **decrease** (negative relationship) |


::: callout-note
#### Understanding Odds and Odds Ratios

Many students find **odds ratios** confusing because they are different from probabilities.

#### Probability

A probability is the chance that an event occurs.

For example, suppose that out of 100 consumers:

- 80 join the green loyalty programme.
- 20 do not join the programme.

The probability of joining is:

$P(\text{Join}) =  = 0.80$ or **80%**.



#### Odds

Odds compare the number of times an event occurs with the number of times it does not occur.

Using the same example:

$\text{Odds}=\frac{p}{1-p}=\frac{80}{100} = \frac{80}{20}=4$

The odds are therefore **4 to 1**, often written as **4:1**.

This means that for every consumer who does not join the programme, four consumers join.

:::


To calculate percentage change from odds ratio:
...
**Guilt** (Exp(B) = 1.718, $p < .001$)

> For every **1-unit increase** in Guilt, the **odds of joining** increase by **71.8%**.

**Attract** (Exp(B) = 2.392, $p < .001$)

> For every **1-unit increase** in Attract, the **odds of joining** increase by **139.2%**.

**Age** (Exp(B) = 0.967, $p < .001$)

> For every **1-year increase** in Age, the **odds of joining** decrease by **3.3%**.

**Gender** (Exp(B) = 1.144, $p = .207$)

> Gender does **not** significantly predict joining. The odds ratio is not interpreted due to non-significance.


***Summary***

- `Guilt`: significant **positive** predictor (+71.8% odds increase)
- `Attract`: significant **positive** predictor (+139.2% odds increase)
- `Age`: significant **negative** predictor (-3.3% odds decrease per year)
- `Gender`: **not** significant ($p = .207$)



### Step 3:  Which Variable Has the Strongest Effect?
...
1. **Perceived attractiveness** is the strongest predictor of joining the programme.
2. **Anticipated guilt** is the second strongest predictor.
3. **Age** has a weaker negative effect.
4. **Gender** does not significantly influence joining behaviour.



### Assessing Multicollinearity

Multicollinearity can also affect logistic regression. When predictors are highly correlated with one another, it becomes difficult to estimate their individual effects accurately.

Unlike multiple regression, SPSS does **not** provide VIF values directly in the Binary Logistic Regression procedure. Therefore, multicollinearity is usually assessed using a separate linear regression with the same set of predictors.

**Method 1: Examine Correlations**

Inspect the correlations among the predictors (`Guilt`, `Attract`, `Age`, and `Gender`).

As a rule of thumb:

- Correlation > .90 suggests a serious multicollinearity problem.
- Correlation > .70 warrants further investigation.

**Method 2: Examine VIF Values**

Because SPSS does not report VIF values in Binary Logistic Regression, run a supplementary linear regression.

Enter any continuous variable as the dependent variable (for example, `Guilt`).

Enter the remaining predictors as independent variables.

Click **Statistics** and select **Collinearity Diagnostics**.

Inspect the VIF values in the Coefficients table.

Interpretation: if $\text VIF > 10$, there is a potentially serious multicollinearity problem.

::: callout-important
Although the VIF values are obtained from a linear regression rather than the logistic regression procedure itself, they are based on the relationships among the predictors and therefore provide a useful diagnostic for multicollinearity in logistic regression.
:::
...
You can write your report as follows:


> A binary logistic regression was conducted to examine factors influencing consumers' decision to join a green loyalty programme. The model was significant, $\chi^2(4)=473.548$, \(p<.001\), indicating that anticipated guilt, perceived attractiveness, age, and gender collectively improved prediction of joining behaviour. The model explained approximately 21.8% of the variation in joining behaviour (Nagelkerke \(R^2=.218\)). Overall classification accuracy was 88.7%. However, this result should be interpreted with caution because the model correctly identified 99.1% of consumers who joined the programme but only 9.5% of consumers who did not join. Thus, the model was substantially better at predicting programme participation than non-participation. Perceived attractiveness (\(OR=2.392\), \(p<.001\)) emerged as the strongest predictor, followed by anticipated guilt (\(OR=1.718\), \(p<.001\)). Age had a small but significant negative effect (\(OR=0.967\), \(p<.001\)), whereas gender was not a significant predictor (\(OR=1.144\), \(p=.207\)).
