...
Use the menu options **Analyze**$\rightarrow$**Descriptive statistics**$\rightarrow$**Crosstabs**. Insert `gender` into "Columns" and `q1_online` into "rows". Click `Cells` tab on the left, then `Tick `Row percentages`. Under z-test, tick `Compare columns proportions` and `Adjust p-values (Bonferroni method)`.
